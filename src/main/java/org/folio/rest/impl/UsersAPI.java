    ...
    if(isNull(response)){
      ...
      if(ok){
        ...
        if(statusCode == 404){
          ...
          previousFailure[0] = true;
          asyncResultHandler.handle(io.vertx.core.Future.succeededFuture(
            GetUsersByIdByUseridResponse.withPlainNotFound(message)));
        }
        else if(statusCode == 400){
          logger.error(message);
          previousFailure[0] = true;
          asyncResultHandler.handle(io.vertx.core.Future.succeededFuture(
            GetUsersByIdByUseridResponse.withPlainBadRequest(message)));
        }
        else if(statusCode == 403){
          logger.error(message);
          previousFailure[0] = true;
          asyncResultHandler.handle(io.vertx.core.Future.succeededFuture(
            GetUsersByIdByUseridResponse.withPlainForbidden(message)));
        }
        else{
          logger.error(message);
          previousFailure[0] = true;
...
/*        cf = completedLookup.get("expanded");
  ...
  @Override
  public void postUsersLogin(Boolean expandPerms, List<String> include, LoginCredentials entity,
    ...
    if(entity == null || entity.getUsername() == null || entity.getPassword() == null) {
      asyncResultHandler.handle(io.vertx.core.Future.succeededFuture(
        PostUsersLoginResponse.withPlainBadRequest("Improperly formatted request")));
    } else {
      ...
      .thenAccept((response) -> {
        try {
          ...
          if(!aRequestHasFailed[0]){
            asyncResultHandler.handle(io.vertx.core.Future.succeededFuture(
              PostUsersLoginResponse.withJsonCreated(token, cu)));
          }
        } catch (Exception e) {
          if(!aRequestHasFailed[0]){
            asyncResultHandler.handle(io.vertx.core.Future.succeededFuture(
              PostUsersLoginResponse.withPlainInternalServerError(e.getLocalizedMessage())));
          }
          logger.error(e.getMessage(), e);
        }
      });
